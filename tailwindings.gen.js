...
// Import the necessary functions from culori
import { parse, formatHex } from 'culori';

/**
 * Converts a CSS oklch() string to its hex code.
 * @param {string} oklchString The CSS oklch() color string, e.g., 'oklch(80% 0.15 240)'.
 * @returns {string | null} The hex code, or null if the input is invalid.
 */
function oklchToHex(oklchString) {
  // Parse the CSS string into a culori color object
  const oklchColor = parse(oklchString);

  // Check if the parsing was successful and the color mode is oklch
  if (!oklchColor || oklchColor.mode !== 'oklch') {
    console.error('Invalid OKLCH color string provided.');
    return null;
  }

  // Format the color object as a hex string
  // culori automatically handles the conversion from oklch -> srgb -> hex
  const hexCode = formatHex(oklchColor);

  return hexCode;
}

for (const col in tailwindColorsOriginal) {
    for (const shade in tailwindColorsOriginal[col]) {
        tailwindColorsOriginal[col][shade] = oklchToHex(tailwindColorsOriginal[col][shade]);
    }
}

// write to tailwindings.json
import { writeFileSync } from 'fs';
import { join } from 'path';

const tailwindingsPath = join(__dirname, 'tailwindings.json');
writeFileSync(tailwindingsPath, JSON.stringify(tailwindColorsOriginal, null, 2), 'utf8');